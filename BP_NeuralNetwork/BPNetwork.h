...
// --- -1~1 随机数产生器 --- 
inline double get_11Random()    // 产生-1--1之间的随机小数（精确到小数点后一位） 
{
...
// 3.wDeltaSum: 面对第一层隐含层每个节点权值的delta值累积   delta表示误差△
...
// --- 输出层节点。包含以下数值：--- 
// 1.value:     节点当前值； 
// 2.delta:     与正确输出值之间的delta值；   反向传播误差
// 3.rightout:  正确输出值	期望输出值
// 4.bias:      偏移量	？？？
